...
class RUMMonitorTests: XCTestCase {
    ...
    private var config: RUMConfiguration! // swiftlint:disable:this implicitly_unwrapped_optional
    ...
    override func setUp() {
        ...
        config = RUMConfiguration(
            ...
            viewUpdatesThrottlerFactory: { NoOpRUMViewUpdatesThrottler() } // disable view updates sampling for deterministic behaviour
        )
    }
    ...
    func testLoadingResourceWithURL_thenMarksFirstPartyURLs() throws {
        config.urlSessionTracking = RUMConfiguration.URLSessionTracking(
            firstPartyHosts: .init(
                hostsWithTraceHeaderTypes: ["foo.com": [.datadog]]
            )
        )
        RUM.enable(with: config, in: core)
